...
create table public.company_letters (
  id uuid default gen_random_uuid() primary key,
  company text not null, -- 'TNT', 'HYPE', 'GOAT', 'NOVA'
...
-- Letter Types: read all, write HR/Exec
...
-- Company Letters: read HR/Exec, write HR/Exec
...
-- Payrolls: 
-- staff can read if status is published and belongs to them.
-- HR/Exec can read/write all.
